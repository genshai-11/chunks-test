...
import {
  validateAssessmentChoice,
  getEffectiveAssessmentChoice,
  calculateComponentMastery,
  deriveComponentRawSummaries,
} from '../domain/blue-test/component-engine';
...
const STORAGE_KEYS = {
  ...
  COMPONENT_EVENTS: 'blue_test_component_events',
...
export class BlueTestStorageAdapter {
  ...
  static clearAllHistory(): void {
    ...
    setStorageItem(STORAGE_KEYS.COMPONENT_EVENTS, []);
  }
  ...
  // Component Assessment Events
  static getComponentAssessmentEvents(assignmentId: string): ComponentAssessmentEvent[] {
    const events = getStorageItem<ComponentAssessmentEvent[]>(STORAGE_KEYS.COMPONENT_EVENTS, []);
    return events
      .filter((e) => e.assignmentId === assignmentId)
      .sort((a, b) => a.sequence - b.sequence);
  }

  static getEffectiveComponentAssessmentChoice(
    assignmentId: string,
    questionGlobalOrder: number
  ): ComponentAssessmentChoice {
    const allEvents = this.getComponentAssessmentEvents(assignmentId);
    const qEvents = allEvents.filter((e) => e.questionGlobalOrder === questionGlobalOrder);
    return getEffectiveAssessmentChoice(qEvents);
  }

  static recordComponentAssessment(payload: {
    assignmentId: string;
    attemptId?: string;
    questionGlobalOrder: number;
    ...
    actor?: string;
    reason?: string;
  }): { event: ComponentAssessmentEvent } {
    validateAssessmentChoice(payload.choice, payload.questionGlobalOrder);

    const allEvents = getStorageItem<ComponentAssessmentEvent[]>(STORAGE_KEYS.COMPONENT_EVENTS, []);
    const assignmentEvents = allEvents.filter((e) => e.assignmentId === payload.assignmentId);
    const qEvents = assignmentEvents.filter((e) => e.questionGlobalOrder === payload.questionGlobalOrder);

    // If an existing assessment exists for this question, a non-empty reason is required for correction
    if (qEvents.length > 0) {
      if (!payload.reason || !payload.reason.trim()) {
        throw new Error('Component assessment correction requires a non-empty reason.');
      }
    }

    const nowStr = new Date().toISOString();
    const actor = payload.actor || 'Teacher';
    const sequence = assignmentEvents.length + 1;
    const attemptId = payload.attemptId || `blue-attempt-${payload.assignmentId}-q${payload.questionGlobalOrder}`;

    const event: ComponentAssessmentEvent = {
      id: `blue-comp-evt-${Date.now()}-${Math.random().toString(36).substring(2, 6)}`,
      attemptId,
      assignmentId: payload.assignmentId,
      questionGlobalOrder: payload.questionGlobalOrder,
    ...
    setStorageItem(STORAGE_KEYS.COMPONENT_EVENTS, allEvents);

    return { event };
  }

  static getComponentMastery(assignmentId: string): PercentCPDResult {
    const events = this.getComponentAssessmentEvents(assignmentId);
    const attempts = this.getAttempts(assignmentId);
    return calculateComponentMastery(events, attempts);
  }

  static getComponentRawSummaries(assignmentId: string): ComponentRawSummary[] {
    const events = this.getComponentAssessmentEvents(assignmentId);
    const attempts = this.getAttempts(assignmentId);
    return deriveComponentRawSummaries(events, attempts);
  }
