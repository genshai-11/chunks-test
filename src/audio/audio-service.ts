...
export function playNarrationAssetOrSpeech(
  ...
  if (resolvedUrl) {
    ...
    const handleFailure = (reason: string) => {
      if (hasFailed) return;
      hasFailed = true;
      console.warn(`Gemini audio asset playback failed (${reason}) for: "${text}". Falling back to Web Speech API.`);
  ...
  // If no Gemini TTS audio URL exists or fails, fall back gracefully to Web Speech API
