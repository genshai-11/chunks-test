...
export const BlueTestAnalysis: React.FC<BlueTestAnalysisProps> = ({
  ...
  const attempts = BlueTestStorageAdapter.getAttempts(assignment.id);
  ...
  const finalizedAttemptsForAct = attempts.filter((a) => a.finalizedAt);
  const totalEffectiveElapsed = finalizedAttemptsForAct.reduce((acc, a) => acc + getEffectiveAttemptValues(a).effectiveElapsedSeconds, 0);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 text-slate-900">
      ...
      {/* Summary KPI Cards Grid (4 Cards: %i, RFC, ACT, Completion Rate) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {/* 1. %RFC Metric Card in HOT COLOR STYLING (Rose/Red) */}
        <div className="bg-rose-600 border border-rose-500 rounded-3xl p-6 text-white shadow-lg space-y-3 relative overflow-hidden">
          <div className="flex items-center justify-between">
            <span className="text-sm font-extrabold text-rose-100" title="Reaction Failure Coefficient = 100% - %i">
              %RFC
            </span>
        ...
        {/* 2. %i Metric Card in BLUE STYLING (#2563EB) */}
        <div className="bg-blue-600 border border-blue-500 rounded-3xl p-6 text-white shadow-lg space-y-3 relative overflow-hidden">
          <div className="flex items-center justify-between">
            <span className="text-sm font-extrabold text-blue-100" title="Rules Awareness / Observation %i">
          ...
          <div className="text-xs text-blue-100 font-medium">
            Based on {metrics.finalizedCount}/49 completed questions
          </div>
        </div>

        {/* 3. ACT Card */}
        <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-xs space-y-3">
          <div className="flex items-center justify-between text-slate-700 text-xs font-extrabold uppercase">
            <span title="Average Conscious Time">ACT</span>
            <Activity className="w-4 h-4 text-indigo-600" />
          </div>
          <div className="text-3xl font-black text-slate-900 font-mono">
            {actValue.toFixed(2)}s
          </div>
          <div className="text-xs text-slate-700 font-bold">
            Formula: Total MCT / 49
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-xs space-y-3">
          ...
          <div className="text-xs text-slate-700 font-bold">
            {metrics.completionPercent.toFixed(1)}% of total package
          </div>
        </div>
      </div>
